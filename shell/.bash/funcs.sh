...
deadsym() {
  for i in **/*; do [[ -L $i && ! -e $(readlink -fn "$i") ]] && rm -rfv "$i"; done
}
...
resize() {
  hash convert >/dev/null 2>&1 || {
    printf "This function requires imagemagick\n"
    return 1
  }
  local size="$1"
  shift
...
ii() {
  ...
  echo -e "\n\e[1;31mAdditionnal information:\e[m "
  uname -a
  echo -e "\n\e[1;31mUsers logged on:\e[m "
...
toggle_alacritty_theme() {
  ...
  current_theme=$(basename "$(readlink -f "$theme_file")")

  if [[ "$current_theme" == "gruvbox_dark.toml" ]]; then
    ln -sf "$HOME/.config/alacritty/themes/themes/gruvbox_light.toml" "$theme_file"
    echo "Alacritty cambiado a tema claro"
  else
    ln -sf "$HOME/.config/alacritty/themes/themes/gruvbox_dark.toml" "$theme_file"
    echo "Alacritty cambiado a tema oscuro"
  fi
